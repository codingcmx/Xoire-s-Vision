...
const ProductSchema = z.object({
  ...
  price: z.number(),
  productUrl: z.string().url().optional().describe("URL to the product page, if available."),
});
...
const GenerateProductRecommendationsOutputSchema = z.object({
  products: z
    .array(z.object({
        ...
        productUrl: z.string().url().optional().describe("Direct link to the product page, if available from the catalog tool. If not available, this field should be omitted entirely."),
    }))
    .describe('An array of product objects that are recommended from the catalog. Each object includes the product name, rationale, and productUrl (if provided).'),
  overallReasoning: z
    .string()
    .describe(
...
const getProductCatalogTool = ai.defineTool(
  {
    ...
    description: 'Searches the product catalog based on a query. Use this tool to find products that match user preferences or descriptions before making recommendations. Include product URLs in the results if available.',
...
3.  Use the 'getProductCatalogTool' with your search query to fetch relevant products from our catalog. This tool will provide product details including name, description, and product URL if available.
...
7.  If the 'getProductCatalogTool' provided a valid 'productUrl' for a product, include it in your response.
8.  **VERY IMPORTANT**: If the tool does NOT supply a 'productUrl' for a product, or if the 'productUrl' is not a valid URL, the 'productUrl' field **MUST NOT BE INCLUDED** in the JSON object for that product. Do NOT set it to \`null\`, an empty string, or any placeholder. Only include the 'productUrl' field if a valid URL string is available from the tool.
